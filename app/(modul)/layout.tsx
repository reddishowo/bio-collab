...
import { BookOpen, CalendarDays, Home, LogOut, Users, X } from "lucide-react";
  ...
  return (
    <aside className="h-full flex flex-col w-64 bg-white border-r border-pastel-blue/30 shadow-xl md:shadow-sm">
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-1 text-slate-700">
        <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 mt-2 px-2">Ruang Kerja</p>
        <Link href="/lkpd" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors font-medium ${pathname === '/lkpd' && !searchParams.get("meeting") ? 'bg-pastel-dark text-white shadow-md' : 'hover:bg-pastel-blue/10 hover:text-pastel-dark'}`}>
